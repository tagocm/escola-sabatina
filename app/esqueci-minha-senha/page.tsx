...
export default function ForgotPasswordPage() {
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      setMessage({
        type: "success",
        text: "Enviamos as instruções para o e-mail informado.",
      });
    });
  };
  ...
  return (
    <div className="w-full max-w-md bg-surface border-[4px] border-foreground p-8 md:p-10 shadow-editorial flex flex-col gap-8">
      <div className="flex flex-col gap-3 border-b-4 border-foreground pb-6">
        <div className="flex h-12 w-12 items-center justify-center border-[3px] border-foreground bg-es-blue shadow-editorial-sm">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          <label className="text-sm font-black uppercase tracking-widest" htmlFor="email">
            E-mail
          </label>
          <input
            disabled={isPending}
        ...
        >
          <span>{isPending ? "Enviando" : "Enviar link"}</span>
          {isPending ? (
            <ButtonLoader label="Enviando link" />
          ) : (
      ...
      <Link
        href="/login"
        className="inline-flex h-12 items-center justify-center gap-2 border-[3px] border-foreground bg-surface px-4 text-sm font-black uppercase tracking-widest shadow-editorial-sm hover:shadow-editorial-hover active:translate-y-0.5 active:translate-x-0.5 transition-all"
      >
